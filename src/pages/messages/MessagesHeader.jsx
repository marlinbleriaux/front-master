...
function MessagesHeader({ msgSidebarOpen, setMsgSidebarOpen }) {
  const { currentUser, currentApp } = useSelector((state) => state.auth);
  const { curentChat } = useSelector((state) => state.messages);
  return (
    <div className="sticky top-0 left-0 right-0">
      <div className="flex items-center justify-between bg-slate-100 border-b border-slate-200 px-4 sm:px-6 md:px-5 h-16">
        {/* People */}
        <div className="flex items-center">
          ...
          <div className="flex -space-x-3 -ml-px">
            <a className="block" href="#0">
              <img
                ...
                src={
                  currentUser?.picture?.baseUrl ||
                  `https://ui-avatars.com/api/?name=${currentUser?.lastName?.toString()}&rounded=true`
                }
                width="32"
                height="32"
              />
            ...
            <a className="block" href="#0">
              <img
                className="rounded-full border-2 border-white box-content"
                src={getValues(curentChat, currentUser)?.image}
                width="32"
                height="32"
              />
            </a>
          ...
          <span className="ml-5">
            {getValues(curentChat, currentUser)?.title}
          </span>
        </div>
